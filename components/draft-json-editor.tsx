...
export default function DraftJsonEditor({ isOpen, onClose, plan }: DraftJsonEditorProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!parsedData.metadata.planName || 
        ...
        <DialogContent className="max-w-dialog-lg dialog-content-base flex flex-col">
          ...
          <DialogHeader className="flex-shrink-0">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
              <div>
                ...
                <DialogDescription>
                  Make changes to the draft plan's JSON data. Click Save to update the plan you're editing.
                </DialogDescription>
              </div>
            </div>
          </DialogHeader>
          ...
          <DialogFooter className="dialog-footer-between flex-shrink-0 flex flex-row items-center gap-2">
            ...
            <div className="flex gap-2 justify-end items-center">
              ...
              <Button
                onClick={handleSave}
                disabled={isSubmitting || !jsonText || !!error}
              >
                {isSubmitting ? (
                  <>
                    <svg
