  ...
  persist(
    (set, get) => ({
      ...
      fetchPlanMetadata: async () => {
        try {
          console.log("DEBUG: fetchPlanMetadata called!")
          set({ isLoading: true, error: null })

          // Get current plan metadata list from state
          const currentMetadata = get().planMetadataList

          // Only fetch new data if we don't have any metadata yet
          if (currentMetadata.length === 0) {
            ...
            // Set the fetched metadata only if we don't have any yet
            set({ planMetadataList: planMetadata, isLoading: false })
          } else {
            // If we already have metadata, don't overwrite it
            set({ isLoading: false })
          }
        } catch (err) {
